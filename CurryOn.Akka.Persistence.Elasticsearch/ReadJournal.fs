...
type ElasticsearchReadJournal (system: ExtendedActorSystem) =
    ...
    let readBatchSize = lazy(config.Value.GetInt("read-batch-size", 1024))
    let maxBufferSize = lazy(config.Value.GetLong("max-buffer-size", 4096L))
    ...
    member __.CurrentEventsByPersistenceId (persistenceId, fromSequence, toSequence) =
        Source.ActorPublisher<EventEnvelope>(EventsbyPersistenceIdPublisher.Props(false, persistenceId, fromSequence, toSequence, !maxBufferSize, ElasticsearchJournal.Identifier))
    ...
    member __.CurrentEventsByTag (tag, offset) =
        Source.ActorPublisher<EventEnvelope>(EventsByTagPublisher.Props(false, tag, offset, !maxBufferSize, ElasticsearchJournal.Identifier))
              .MapMaterializedValue(fun _ -> NotUsed.Instance)
              .Named("EventsByTag") |> unbox<Source<EventEnvelope,NotUsed>>

    member __.CurrentPersistenceIds () =
        Source.ActorPublisher<string>(PersistenceIdsPublisher.Props(false, ElasticsearchJournal.Identifier))
              .MapMaterializedValue(fun _ -> NotUsed.Instance)
              .Named("CurrentPesistenceIds") |> unbox<Source<string,NotUsed>>

    member __.AllPersistenceIds () =
        Source.ActorPublisher<string>(PersistenceIdsPublisher.Props(true, ElasticsearchJournal.Identifier))
              .MapMaterializedValue(fun _ -> NotUsed.Instance)
              .Named("AllPesistenceIds") |> unbox<Source<string,NotUsed>>

    member __.EventsByPersistenceId (persistenceId, fromSequence, toSequence) =
        Source.ActorPublisher<EventEnvelope>(EventsbyPersistenceIdPublisher.Props(true, persistenceId, fromSequence, toSequence, !maxBufferSize, ElasticsearchJournal.Identifier))
              .MapMaterializedValue(fun _ -> NotUsed.Instance)
              .Named("EventsByPersistenceId") |> unbox<Source<EventEnvelope,NotUsed>>

    member __.EventsByTag (tag, offset) =  
        Source.ActorPublisher<EventEnvelope>(EventsByTagPublisher.Props(true, tag, offset, !maxBufferSize, ElasticsearchJournal.Identifier))
              .MapMaterializedValue(fun _ -> NotUsed.Instance)
              .Named("EventsByTag") |> unbox<Source<EventEnvelope,NotUsed>>
